...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  const id = (request.url as string)
    .split('/')
    .filter((p) => p)
    .reverse()[0]
    .split('?')[0]
  ...
  response.setHeader('content-type', 'text/html').status(200).send(/* html */ `
      <!DOCTYPE html>
      <html lang="en">
        <head>
          <meta charset="UTF-8" />
          ...
          <meta property="og:url" content="https://https://notion-vite-vue3-ts-nine.vercel.app/workspaces/${id}" />
          <meta property="twitter:card" content="summary" />
          <meta property="twitter:site" content="Notion Clone!" />
          <meta property="twitter:title" content="${title}" />
          <meta property="twitter:description" content="${content}" />
          <meta property="twitter:image" content="${poster}" />
          <meta property="twitter:url" content="https://https://notion-vite-vue3-ts-nine.vercel.app/workspaces/${id}" />
